...
void light_setup()
{
    ...
    xTaskCreatePinnedToCore(light_task, "Light Task", 4096, NULL, light_task_Priority, &LightTask, 1);      
}
...
void light_task(void *pvParameters)
{
    for (;;)
    {
        ...
        // Non-blocking queue ingestion to keep state machine fluid
        if (xQueueReceive(Light_Queue, &parts, 0) == pdTRUE)
        {
            ...
            if (targetPin < 16)
            {
                if (identifier.compare("LBLINK") == 0)
                {
                    ...
                    writeLEDIntensityDirect(targetPin, LEDON);
                }
                ...
                else if (identifier.compare("LLEDONOFF") == 0) TurnLEDOnOff(targetPin, parts.value2);
                else if (identifier.compare("LLEDINTENSITY") == 0) TurnLEDOnWithIntensity(targetPin, parts.value2);
                else if (identifier.compare("LLEDALLOFF") == 0) { for (int i=0; i<16; i++) TurnLEDOnOff(i, 0); }
                else if (identifier.compare("LLEDALLON") == 0) { for (int i=0; i<16; i++) TurnLEDOnOff(i, 1); }
            }
        }
        ...
        for (uint32_t pin = 0; pin < 16; pin++)
        {
            if (LEDs[pin].state == blink)
            {
                ...
                if (animTracks[pin].blinkState && elapsed >= LEDs[pin].OnTimeMS)
                {
                    animTracks[pin].blinkState = false;
                    animTracks[pin].lastUpdateTime = now;
                    writeLEDIntensityDirect(pin, LEDOFF);
                }
                else if (!animTracks[pin].blinkState && elapsed >= LEDs[pin].OffTimeMS)
                {
            ...
            else if (LEDs[pin].state == breathe)
            {
                ...
                switch (animTracks[pin].phase)
                {
                    case PHASE_RISE:
                    {
                        ...
                        else writeLEDIntensityDirect(pin, 255 - ((255 * elapsed) / duration));
                        break;
                    }
                    case PHASE_ON:
                    ...
                    case PHASE_FALL:
                    {
                        ...
                        else writeLEDIntensityDirect(pin, (255 * elapsed) / duration);
                        break;
                    }
                    case PHASE_OFF:
        ...
        vTaskDelay(pdMS_TO_TICKS(10)); // Clean 100Hz scheduler yield
    }
}
...
static void writeLEDIntensityDirect(uint32_t pin, int intensity)
{
    ...
    if (animTracks[pin].lastWrittenIntensity == intensity) return;
...
{
    if (pin >= 16) return;
    LEDs[pin].state = (OnOff == 1) ? on : pinState::off;
    animTracks[pin].initialized = false;
    writeLEDIntensityDirect(pin, (OnOff == 1) ? LEDON : LEDOFF);
}

void stopCurrentTaskOnPin(uint32_t pin) {} // Retained for signature validation compatibility
